...
export function resetProgress(){localStorage.removeItem(KEY);for(let i=localStorage.length-1;i>=0;i--){const key=localStorage.key(i);if(key?.startsWith('sysadmin-lab-section-'))localStorage.removeItem(key)}}
